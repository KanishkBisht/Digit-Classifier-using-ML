...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            tflite = new Interpreter(modelLoading());
        } catch (Exception ex) {
            ex.printStackTrace();
        }

        resPreview = findViewById(R.id.resPreview);
        resPreview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try {
                    displayScreen.setDrawingCacheEnabled(true);
                    displayScreen.buildDrawingCache();
                    Bitmap bitmap = displayScreen.getDrawingCache().copy(Bitmap.Config.RGB_565, false);
                    displayScreen.setDrawingCacheEnabled(false);
        ...
        displayScreen.setLayoutParams(new ConstraintLayout.LayoutParams(width, width));
        DisplayMetrics metrics = new DisplayMetrics();
        getWindowManager().getDefaultDisplay().getMetrics(metrics);
        displayScreen.init(metrics);
    ...
    private MappedByteBuffer modelLoading() throws IOException {
    ...
    private ByteBuffer bitmapToModelsMatchingByteBuffer(Bitmap bitmap) {
        ...
        for (int i = 0; i < SIZE; ++i) {
            for (int j = 0; j < SIZE; ++j) {
                ...
                for (float channelVal : modifyPixel(pixelVal)) {
    ...
    private float[] modifyPixel(int pixel) {
